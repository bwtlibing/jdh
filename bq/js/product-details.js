...
window.onload=function () {
    (function () {
        ...
        function productDetailHtml(data) {/*商品详情赋值*/
            ...
            isVideo=data.videoCode;
            if (data.videoCode) {/*视频*//*测试完需要放开*/
                var videoCode=data.videoCode;

                playPrepare(videoCode, 160, 230, 76, 115);/*这里要改的*/

                //小图片
                // plyIfr.src='../../bq/img/video.mp4';
                imgChangeStatus();
                smallImg[0].src='../../img/videoTip.png';
            } else {

                bigImg.src = data.images[0];
            ...
            function imgChangeStatus() {
                for (var i = 0; i < 4; i++) {
                    ...
                    if(data.images[i]){
                        smallImgHtml='<img class="img-item menu-item small-img" src="'+ data.images[i]+'" alt="">';
                    }else {
                ...
                if( data.images.length>4){
                    ...
                    if(data.images.length==6){
                        ...
                        smallImgBox.innerHTML+='<img class="img-item menu-item small-img" src="'+data.images[4]+'" alt="">'
            ...
            if (data.seller) {
                ...
                sellerImage.src = data.seller.image;
                ...
                if(smallImg.length>4){
                    if(dataImgLength==6){
                        if(i==4){
                            ...
                            // smallImg[i-2].style.display='inline-block';
                            // smallImgContainer.style.left=-92+"px";
                        }else if(i==3) {
                            smallImg[0].style.display='inline-block';
                            smallImg[smallImg.length-1].style.display='none';
                            smallImg[smallImg.length-2].style.display='none';
                            // smallImg[i-1].style.display='inline-block';
                            // smallImgContainer.style.left=0+"px";
                        }
